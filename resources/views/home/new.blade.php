...
@section('content')
  <article class="blogs">
    <h1 class="t_nav"><span>目前的位置：<a href="{{url('/')}}">首頁</a>&nbsp;&gt;&nbsp;<a href="{{url('cate/'.$field->cate_id)}}">{{$field->cate_name}}</a></span><a href="{{url('/')}}" class="n1">網站首頁</a><a href="{{url('cate/'.$field->cate_id)}}" class="n2">{{$field->cate_name}}</a></h1>
    <div class="index_about">
      <h2 class="c_titile">{{$field->art_title}}</h2>
      <p class="box_c"><span class="d_time">公開時間：{{date('Y-m-d',$field->art_time)}}</span><span>編輯：{{$field->art_editor}}</span><span>查看次數：{{$field->art_view}}</span></p>
      <ul class="infos">
        {!! $field->art_content !!}
      </ul>
      <div class="keybq">
